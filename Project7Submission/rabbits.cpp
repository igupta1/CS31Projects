...
int getNumRabbitsAroundNSWE(const Arena& a, int r, int c);
...
void Arena::display(string msg) const
{
	...
	for (r = 1; r <= rows(); r++)
	{
		for (c = 1; c <= cols(); c++)
		{
			...
			{
				displayGrid[r-1][c-1] = char(int('0') + numberOfRabbitsAt(r, c));
			}
...
int getNumRabbitsAroundNSWE(const Arena& a, int r, int c)
{
	...
	if (r >= 2)
	{
		total += a.numberOfRabbitsAt(r-1, c);
	}
	if (r <= a.rows()-1)
	{
		total += a.numberOfRabbitsAt(r+1, c);
	...
	if (c >= 2)
	{
		total += a.numberOfRabbitsAt(r, c-1);
	}
	if (c <= a.cols()-1)
	{
		total += a.numberOfRabbitsAt(r, c+1);
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
	...
	//if rabbits can posibly kill then move into spot least likely to get taken by rabbits
	//if saying put is safe, the user should stay or move into another safe position
	//if all positions are dangerous then go where least rabbits can attack
	
	
	//cout << "NUMBER OF RABBITS AROUND PLAYER IS " << getNumRabbitsAroundNSWE(a, r, c) << endl;
	
		
	int rabNumber = MAXRABBITS + 1;
	int dir = -1;
	
	if ((r >= 2) && (a.numberOfRabbitsAt(r-1,c) == 0))
	{
		int temp = getNumRabbitsAroundNSWE(a, r-1, c);
		if (temp < rabNumber)
		{
			rabNumber = temp;
			dir = NORTH;
	...
	if ((r <= a.rows()-1) && (a.numberOfRabbitsAt(r+1,c) == 0))
	{
		int temp = getNumRabbitsAroundNSWE(a, r+1, c);
		if (temp < rabNumber)
		{
	...
	if ((c >= 2) && (a.numberOfRabbitsAt(r,c-1) == 0))
	{
		int temp = getNumRabbitsAroundNSWE(a, r, c-1);
		if (temp < rabNumber)
		{
			rabNumber = temp;
			dir = WEST;
	...
	if ((c <= a.cols()-1) && (a.numberOfRabbitsAt(r,c+1) == 0))
	{
		int temp = getNumRabbitsAroundNSWE(a, r, c+1);
		if (temp < rabNumber)
		{
	...
	int temp = getNumRabbitsAroundNSWE(a, r, c);
	if (temp < rabNumber)
	{
		rabNumber = temp;
		dir = -1;
